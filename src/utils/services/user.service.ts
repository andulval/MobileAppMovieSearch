...
const usersApi = baseApi
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      getSearchMovies: build.query<MovieDetailsProps[], string>({
        query: (searchQuery = "") =>
      ...
      getPopularMovies: build.query<MovieDetailsProps[], void>({
        query: () => `popular?language=en-US&page=1${MY_TMDB_KEY}`,
        // Provided tag to re-fetch the data if the specified tag is invalidated.
